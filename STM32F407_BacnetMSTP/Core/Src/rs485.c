...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	//(void)FIFO_Put(&Receive_Buffer, UartDMABuffer);
	 //RS485_Receive_Bytes++;
	 if (!Transmitting) {
	       FIFO_Put(&Receive_Queue, UartDMABuffer);
	       RS485_Receive_Bytes++;
	 }
	 if(RS485_Receive_Bytes==2){
...
void rs485_bytes_send(const uint8_t *buffer, uint16_t nbytes)
{
    if (buffer && (nbytes > 0)) {
        if (FIFO_Add(&Transmit_Queue, buffer, nbytes)) {
            ...
            HAL_UART_Transmit_DMA(&huart2, (uint8_t*)buffer, nbytes);
...
//    USART_ITConfig(RS485_USARTx, USART_IT_RXNE, ENABLE);
	FIFO_Init(&Receive_Queue, Receive_Queue_Data, NEXT_POWER_OF_2(DLMSTP_MPDU_MAX));
	FIFO_Init(&Transmit_Queue,Transmit_Queue_Data,NEXT_POWER_OF_2(DLMSTP_MPDU_MAX));
